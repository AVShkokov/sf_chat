    ...
    m_database(database)
{
    ...
    connect(timer, &QTimer::timeout, this, &MainWindow::getStatus);
    timer->start(100);
}
...
void MainWindow::getStatus(){
    ...
    if (!query.empty()) {
        if(m_user.GetBanStatus() != query[4].toInt()) {
            m_user.SetBanStatus(query[4].toInt());

            m_user.GetBanStatus() == 1 ? showErrorMessage("Account banned!") : showInformationMessage("Account an banned!");
        }
...
{
    showInformationMessage("Type \"@name\" to send private message");
}
